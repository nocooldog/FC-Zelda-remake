...
### 高优先级（风格样板前需要）
1. **美术来源**：✅ **自绘**（参考 GBA 缩小帽） — @ben-gao 拍板
2. **像素尺寸**：✅ **GBA 240×160**（不是 NES 256×224）— @ben-gao 拍板

### 中优先级（可推迟）
3. **音效库选型**：✅ **GBA 风格** — @ben-gao 拍板；具体选型（自采样 / 开源 / 委托）后续定
4. **NES ROM 拆解**：是否上传用于参考一代规则（V0.1 §5 列了"使用条件"待确认）
5. **窗口放大选项**：Surface Pro 8 上看着小，是否提供 2x/3x 缩放？（如果像素改到 240×160 实际显示更小，可能需要）
